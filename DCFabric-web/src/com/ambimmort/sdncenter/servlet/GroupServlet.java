    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if ("/add".equals(action)) {
                ...
                magService.addGroup(ip, port, content);
                ...
                if ("all".equals(ip)) {
                    ...
                    while (it.hasNext()) {
                        ...
                        arr.addAll(magService.queryGroupTable(controller.getString("ip"), controller.getString("port")));
                    }
                } else {
                    arr = magService.queryGroupTable(ip, port);
                }
                printInfo(0, arr, out);
            } else if ("/clearGroup".equals(action)) {
                String content = request.getParameter("switchid");
                boolean flag = magService.clearGroup(ip, port, content);
                printInfo(0, flag, out);
            } else if ("/del".equals(action)) {
                String content = request.getParameter("content");
                boolean flag = magService.removeGroupTable(ip, port, content);
                printInfo(0, flag, out);
            }
        } catch (Exception ex) {
