...
typedef struct packet_in_info
{
    ...
    UINT4 inport;       //入口交换机端口号
    UINT4 data_len;     //以太网数据包长度
    UINT1 *data;        //以太网数据包
}packet_in_info_t;
...
{
    UINT4 macuser_hsize;            //指定用户哈希表大小  读配置
    UINT4 macuser_hsize_tot;        //指定用户哈希表大小  读配置
    UINT4 macuser_lifetime;         //指定用户的生存时间  读配置  其实是SDN交换机的hard time
    mac_user_t **macuser_tb;        //分配存储哈希表首地址的内存
    void *macuser_memid;            //内存池标志
    pthread_mutex_t macuser_mutex;  //全局表 查询时加锁
    void *macuser_timer;            //定时器标志
    UINT4 macuser_cnt;              //记录MAC 地址用户数
}mac_user_table_t;


struct mac_user
{
    struct gn_switch *sw;     //所在交换机
    UINT4 port;          //该MAC地址对应的端口  主机字节序
    INT4 tenant_id;      //标识该用户属于哪个租户网络
    UINT4 ipv4;          //ip,主机字节序
    UINT1 ipv6[16];      //ipv6,主机字节序
    UINT1 mac[6];        //用户MAC地址
    time_t tv_last_sec;  //最后更新的时间
    void *timer;         //关联的定时器
...
{
    UINT1 *start_pos;        //处理后，下次处理起始位置
    UINT1 *buff;
    UINT4 len;
    UINT4 bak_len;           //上个buffer未处理掉的长度
}buffer_cache_t;
...
{
    struct gn_switch *neigh_sw;     //相邻的交换机
    struct gn_port *neigh_port;  //与相邻交换机直连的端口
}neighbour_t;
...
typedef struct gn_port
{
    ...
    port_stats_t stats;     //port实时吞吐量信息
} gn_port_t;
...
typedef struct gn_switch
{
    ...
    pthread_t pid_recv;   //收包
    pthread_t pid_proc;   //处理包
