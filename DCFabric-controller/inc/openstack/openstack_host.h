...
//	UINT1 mac[6];
//	UINT4 ofport_no;
	char tenant_id[OPENSTACK_TENANT_ID_LEN];				//by:yhy 租户ID
	char network_id[OPENSTACK_NETWORK_ID_LEN];				//by:yhy
	char subnet_id[OPENSTACK_SUBNET_ID_LEN];				//by:yhy
	char port_id[OPENSTACK_PORT_ID_LEN];					//by:yhy
	UINT2 security_num;
	UINT1* security_data;
}openstack_port,* openstack_port_p;

//by:yhy Openstack子网结构体
typedef struct _openstack_subnet{
	...
	UINT1 start_ipv6[16];
	UINT1 end_ipv6[16];
	char cidr[OPENSTACK_CIDR_ID_LEN];					//by:yhy 无类别域间路由
	void* gateway_port;
	void* dhcp_port;
	UINT4 port_num;
	UINT1 external;
