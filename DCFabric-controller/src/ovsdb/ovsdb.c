...
UINT1 g_ovsdb_of_version = OFP13_VERSION;           //openstack ovs的openflow版本
UINT1 g_tunnel_type = NETWORK_TYPE_VXLAN;           //openstack网络类型 gre/vxlan

UINT1 g_ovsdb_clients[OVSDB_MAX_CONNECTION] = { 0 };
UINT1 g_ovsdb_clients_bak[OVSDB_MAX_CONNECTION] = { 0 };    //clients的备份  用于删除端口连接的交换机
UINT4 g_ovsdb_clients_ip[OVSDB_MAX_CONNECTION] = { 0 };

ovsdb_server_t g_ovsdb_nodes[OVSDB_MAX_CONNECTION];
UINT4 g_ovsdb_port = OVSDB_SERVER_PORT;    //ovsdb接口，默认端口6640
...
void *ovsdb_recv_msg(void *para)
{
    ...
    while (1)
    {
        ...
        if (FD_ISSET(g_ovsdb_sockfd, &recvmask))
        {
            ...
            conn_fd = accept(g_ovsdb_sockfd, (struct sockaddr *) &g_ovsdb_addr, &size);    //长连接其实只accept一次
            clientip = *(UINT4 *) &g_ovsdb_addr.sin_addr;    //网络字节序
