...
INT4 fabric_openstack_floating_ip_packet_out_handle(p_fabric_host_node src_port, packet_in_info_t *packet_in, external_floating_ip_p fip, param_set_p param_set)
{
	...
	LOG_PROC("INFO", "Floating: switch path is not exist!");
	return IP_DROP;
}
//by:yhy 判断host_p与fixed_port是否相等
...
//by:yhy 判断host_p在switch里是否正在使用中
...
//by:yhy 根据给定参数,执行操作(1create2delete)
...
//by:yhy 对fixed_port的subnet执行type操作  why?
INT4 process_floating_internal_subnet_flow(p_fabric_host_node fixed_port, openstack_subnet_p subnet, INT4 type)
{
	...
	if (OPENSTACK_PORT_TYPE_HOST == fixed_port->type) 					//by:yhy 主机类型节点
	{
		update_floating_internal_subnet_flow(fixed_port, type, subnet->cidr, subnet->external);
	}
    else if (OPENSTACK_PORT_TYPE_LOADBALANCER == fixed_port->type) 		//by:yhy 负载平衡类型节点
	{
       openstack_lbaas_pools_p pool_p = find_openstack_lbaas_pool_by_ip(fixed_port->ip_list[0]);
       if (pool_p) 
	   {
...
//by:yhy 对subnet执行type类型操作(1创建2删除)
INT4 process_floating_internal_subnet_flow_by_subnet(openstack_subnet_p subnet, INT4 type)
{
	...
	while (NULL != node_p) 
	{
		//by:yhy 遍历g_openstack_floating_list
...
//by:yhy type(1添加2删除)根据fixed_port处理内部子网流表
...
//by:yhy type(1添加2删除) 根据pool_ip处理floating_lbaas_group_flow
...
//by:yhy 根据pool_ip删除floating_lbaas_flow
...
//by:yhy 在lbaas_group_flows装入floatingip信息
INT4 create_proactive_floating_with_lbaas_group_flows(gn_switch_t* ext_sw, p_fabric_host_node fixed_port, 
                                                      external_port_p ext_port, UINT4 floatingip)
{
...
//by:yhy 在lbaas_group_flows删除floatingip信息
INT4 remove_proactive_floating_with_lbaas_group_flows(gn_switch_t* ext_sw, p_fabric_host_node fixed_port, 
                                                      external_port_p ext_port, UINT4 floatingip)
{   
    if (NULL == ext_sw) 
...
INT4 create_proactive_floating_with_host_flows(p_fabric_host_node fixed_port, gn_switch_t* ext_sw, external_port_p ext_port, UINT4 floatingip)
{
	...
	//by:yhy 查找从ext_port到fixed_port应该从哪个port出去
...
{   
	...
	gn_switch_t* ext_sw = NULL;
	p_fabric_host_node fixed_port = NULL;
    //by:yhy 查找external_port_p
	ext_port = find_openstack_external_by_floating_ip(fip->floating_ip);
	if (ext_port)
	{
		//by:yhy 根据external_port_p查找gn_switch_t
		ext_sw = find_sw_by_dpid(ext_port->external_dpid);
        if (ext_sw) 
		{
			//by:yhy 删除流表
...
//by:yhy 遍历g_openstack_floating_list,并针对各个浮动IP下发流表
void proactive_floating_check_tx_timer(void *para, void *tid)
{
	LOG_PROC("TIMER", "proactive_floating_check_tx_timer - START");
	if (0 == g_openstack_on) 
	{
		return;
	}
   ...
   if (OFPCR_ROLE_SLAVE == g_controller_role) 
   {
	...
	LOG_PROC("TIMER", "proactive_floating_check_tx_timer - STOP");
}
//by:yhy 定时检查浮动IP列表并下发流表
