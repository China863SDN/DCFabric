...
//by:yhy 其中每个成员都启动了一个lbaas_listener_thread线程
...
//markby:yhy 根据pool_id在g_openstack_lbaas_pools_list查找对应openstack_lbaas_node_p节点的->data  openstack_lbaas_pools_p
openstack_lbaas_pools_p find_openstack_lbaas_pool_by_pool_id(char* pool_id)
{
	openstack_lbaas_pools_p lb_pool = NULL;
...
//markby:yhy this function is used to find lbaas pool by ip
...
void* lbaas_listener_thread(void* param)
{
	prctl(PR_SET_NAME, (unsigned long) "LbaasListener" ) ;  
	...
	if (listener_p) 
	{
		while (1)
		{
			...
			MsecSleep(listener_p->check_frequency*1000);
...
//by:yhy 启动lbaas监听
//为g_openstack_lbaas_listener_list->data中每个成员都启动了一个lbaas_listener_thread线程
...
//by:yhy 复位g_openstack_lbaas_pools_list/g_openstack_lbaas_members_list/g_openstack_lbaas_listener_list中节点check_status
...
//markby:yhy 移除g_openstack_lbaas_pools_list中各项
void remove_openstack_lbaas_pool_by_check_status()
{
	...
	//by:yhy 遍历g_openstack_lbaas_pools_list中节点,移除check_status== (UINT2)CHECK_UNCHECKED节点
	while (next_p) 
	{
	...
	//by:yhy 访问g_openstack_lbaas_pools_list中头节点,移除check_status== (UINT2)CHECK_UNCHECKED头节点
...
//markby:yhy 移除g_openstack_lbaas_pools_list->data->pool_member_list中各节点
...
//by:yhy 移除g_openstack_lbaas_listener_list各节点
...
//by:yhy 移除g_openstack_lbaas_members_list->data各节点
...
//by:yhy 移除g_openstack_lbaas_listener_list->data->listener_member_list各节点
...
//by:yhy 根据lb_member复位g_openstack_lbaas_pools_list中openstack_lbaas_node_p节点->data属性对应openstack_lbaas_pools_p->group_flow_installed 
void reset_floating_lbaas_group_flow_installed_flag(openstack_lbaas_members_p lb_member)
{
    if (lb_member) 
	{
