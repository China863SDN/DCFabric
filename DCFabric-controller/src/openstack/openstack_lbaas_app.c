			...
			while ((lb_p) && (lb_p->member_p))
			{
				...
				if (NULL == lb_p->dst_port) {
                    ...
                    if (LBAAS_LISTENER_PING == listener_p->type) {  
                        ...
                        create_tcp_syn_packet(ntohl(g_controller_ip), lb_p->dst_ip, lb_p->protocol_port, g_controller_mac, lb_p->dst_port->mac,
                        ...
                        create_tcp_syn_packet(ntohl(g_controller_ip), lb_p->dst_ip, 80, g_controller_mac, lb_p->dst_port->mac,
                              lb_p->dst_port->sw, lb_p->dst_port->port, lb_p->seq_id);
                    }
                    else {
                        // do nothing
                ...
                if ((LBAAS_LISTENER_PING == type) && (type == listener_p->type)) {
                    ...
                    if ((lb_p->dst_port == dst_port) && (lb_p->seq_id == seq_id - 1) && (lb_p->protocol_port == port_no))
                    {
                        ...
                        if (0x12 == code) {
                            lb_p->wait_status = 0;
							// create RST packet
                            create_tcp_rst_packet(ntohl(g_controller_ip), lb_p->dst_ip, lb_p->protocol_port, g_controller_mac, 
