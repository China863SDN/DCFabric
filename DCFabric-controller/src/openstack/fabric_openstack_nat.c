...
// 定义全局的port总数
UINT4 g_nat_port_total_count = 0;
...
void remove_nat_all_port(nat_port_p head_p)
{
	...
	nat_port_p next_p = head_p->next;

	// 循环判断
	while (NULL != next_p) {
		// LOG_PROC("INFO", "NAT: Remove port id :%d", next_p->external_port_no);
		temp_p = next_p->next;
		g_nat_port_total_count--;
		mem_free(g_nat_port_id, next_p);
		next_p = NULL;
		next_p = temp_p;
	}
...
//by:yhy 查找外部IP=externalip的NAT列表中,是否存在内部IP为internal_ip的相关映射,返回映射数量,并将相关信息存入port_list,externalip_list,proto_list
UINT4 find_nat_port_by_internal_ip(UINT4 internal_ip, UINT2* port_list, UINT4* externalip_list,
								   UINT2* proto_list, UINT4 externalip, UINT4 port_number, 
								   UINT2 proto, nat_port_p head_p)
{
	nat_port_p prev_p = head_p;
	nat_port_p next_p = prev_p->next;
...
//by:yhy 获取internal_ip有多少个NAT连接,返回连接数,并将相关信息存入port_list,externalip_list,proto_list
...
INT4 fabric_openstack_ip_nat_comute_foward(gn_switch_t *sw, packet_in_info_t *packet_in, UINT1 from_inside, param_set_p param_set)
{
	...
	else if (IPPROTO_UDP == packetin_ip->proto)
	{
		...
		//LOG_PROC("INFO", "fabric_openstack_ip_nat_handle: not tcp and udp!");
		return IP_DROP;
	}
...
INT4 fabric_openstack_nat_icmp_comute_foward(gn_switch_t *sw, packet_in_info_t *packet_in, UINT1 from_inside, param_set_p param_set)
{
	...
	if (TRUE == from_inside) {
		...
		if (NULL == nat_icmp_p) {
			LOG_PROC("INFO", "NAT: icmp: nat_icmp_p is NULL!");
			return IP_DROP;
		}
