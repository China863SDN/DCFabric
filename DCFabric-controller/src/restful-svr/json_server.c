...
//by:yhy 存储用户通过restful api自定义的流表 
...
static INT1 *get_switch_info(const const INT1 *url, json_t *root)
{
    ...
    for (i_sw=0; i_sw < g_server.max_switch; i_sw++)
    {
...
static INT1 *get_all_flow_stats(const INT1 *url, json_t *root)
{
    ...
    {
        if (INITSTATE  != g_server.switches[num].conn_state)
        {
        ...
        if ( INITSTATE != g_server.switches[i].conn_state )
        {
            ...
            if (sw)
            {
                for (j = 0; j < sw->n_ports; j++)
                {
                   ...
                   if (sw->neighbor[j]->bValid && sw->neighbor[j]->sw)
                    {
                        entry = json_new_object();
                        port = &(sw->ports[j]);
...
static INT1 *json_fabric_parse_flow(json_t *Obj, stats_fabric_flow_t *list)
{
    ...
    {
        if (INITSTATE != list->sw->conn_state)
        {
            sw_obj = json_new_object();
            json_insert_child(sw_array, sw_obj);
...
static INT1 *get_fabric_switch_info(const INT1 *url, json_t *root)
{
    ...
    {
        sw = &g_server.switches[i];
        if (node_id == sw->dpid &&( INITSTATE != sw->conn_state))
        {
            break;
        }
    }
...
{   
    ...
    {
        sw = &g_server.switches[i];
        if (dpid == sw->dpid && (INITSTATE != sw->conn_state))
        {
            break;
        }
    }
...
static INT1 *del_fabric_flow_entries(const INT1 *url, json_t *root)
{
    ...
    for (; i < g_server.max_switch; i++)
    {
        if ((INITSTATE != g_server.switches[i].conn_state) && dpid == g_server.switches[i].dpid)
        {
            sw = &g_server.switches[i];
            break;
        }
...
INT1 *proc_restful_request(UINT1 type, const INT1 *url, json_t *root)
{
	...
	//by:yhy 遍历restful handler,查找对应处理函数并执行
...
//by:yhy 把Restful API指令,匹配的URL,和对应的响应函数匹配到一起
INT4 register_restful_handler(UINT1 type, const INT1 *url, restful_handler_t restful_handler)
{
	...
	//by:yhy 匹配已经初始化的RESTFUL_Handlers
    for (; i < REST_CAPACITY; i++)
    {
        if (p_handles[i].used == 1)
        {
			//by:yhy why? strncmp已经比较url, p_handles[i].url一致,为何仍执行memcpy(p_handles[i].url, url, strlen(url))?&&后使用等号,确认是否有误
            if ((strncmp(url, p_handles[i].url, p_handles[i].url_len) == 0)&& (p_handles[i].url_len = strlen(url)))
            {
	...
	//by:yhy 匹配未经初始化的RESTFUL_Handlers
