...
//by:yhy 将host主机信息(所有IP,dpid,port)存入Hbase
...
//by:yhy  持久化即将有效的openstack的外部端口信息保存至配置文件
...
void persist_fabric_nat_icmp_iden_list()
{
	...
	if( bSynFlag)
	{
		persist_data(NAT_ICMP, OPERATE_ADD, g_filed_num_master, g_filed_pad_master);
	}
}
...
//by:yhy 将g_nat_host_list_head存入Hbase
void persist_fabric_nat_host_list()
{
	BOOL bSynFlag = FALSE ;
	nat_host_p node = NULL;
	nat_host_p host_p = NULL;
	nat_port_p tcp_node = NULL;
	nat_port_p udp_node = NULL;
    LOG_PROC("INFO", "Ready to sync nat host.");
    if (NULL != g_nat_host_list_head)
    {
    	node = g_nat_host_list_head->next;
		...
		while(NULL != node)
		{
		    ...
		    g_filed_num_master++;
			//tcp list
		    tcp_node = host_p->tcp_port_list->next;
		    if (NULL != tcp_node)
		    {
		        //save tcp port type
			...
			//udp list
		    udp_node = host_p->udp_port_list->next;
		    if (NULL != udp_node)
		    {
		        //save tcp port type
		...
		if( bSynFlag)
		{
			persist_data(NAT_HOST, OPERATE_ADD, g_filed_num_master, g_filed_pad_master);
		}
    }
}
...
//by:yhy 将g_openstack_lbaas_members_list存入Hbase
